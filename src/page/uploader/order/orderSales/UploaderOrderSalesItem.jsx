import {CheckOutlined, CloseOutlined, EditFilled, PrinterOutlined} from "@ant-design/icons";
import {
  Button, Card, List, Popconfirm, Skeleton, Table, Tag, Tooltip, message,
} from "antd";
...
} from "../../../../features/uploader/order/orderOfSales/uploaderOrderOfSalesApiSlice";
...
import {UploaderProducts} from "../../index";
...
function UploaderOrderSalesItem(props) {
  ...
  const [printData, setPrintData] = useState({
    orderId: null, productList: [], customer: {
      name: "", balance: "", allTelefon: [],
    }, date: "", chekId: "",
  });
  ...
  useEffect(() => {
    if (product_list && product_list?.length) {
      setTableData(product_list.map((item, index) => ({
        ...item, id: uuidv4(), index: index + 1,
      })));
    }
  }, [product_list]);
  ...
  const handleCloseOrder = async (orderId) => {
    ...
    try {
      ...
      if (resData?.success === true) {
        const {
          product_list, old_client_balans, client, after_balans, vaqt, all_summa, dostavchik, dostavchik_telefon, agent,
        } = resData.data;

        const newPrintData = {
          orderId: orderId, productList: product_list, customer: {
            ...
            debt: client?.balans,
          }, afterBalance: after_balans, date: vaqt, allPoductTotalPrice: all_summa, supplier: {
            name: dostavchik, telefon: dostavchik_telefon,
          }, agent,
        };
        ...
        if (resData?.message) {
          messageApi.open({
            key, type: "success", content: resData?.message,
          });
        }
      } else if (resData?.success === false) {
        if (resData?.message) {
          messageApi.open({
            key, type: "error", content: resData?.message,
          });
        }
      }
    } catch (err) {
      if (err.status === "FETCH_ERROR") {
        messageApi.open({
          key, type: "warning", content: `Ulanishda xatolik! Qaytadan urinib ko'ring!`,
        });
      }
    } finally {
  ...
  const handleOpenCloseOrderItemModal = ({salesOrderId, salesOrderItemId}) => {
  ...
  return (<>
    ...
    >
      <Table
        scroll={{x: 600}}
        columns={[{
          title: "T/r", dataIndex: "index", key: "index",
        }, {
          title: "Nomi", dataIndex: "product_name", key: "product_name",
        }, {
          title: "Massa(kg)", dataIndex: "massa", key: "massa",
        }, {
          title: "Summasi(so'm)", dataIndex: "summa", key: "summa",
        }, {
          title: "Tayyorlandi(kg)", dataIndex: "tayyorlandi", key: "tayyorlandi",
        }, {
          title: "Status", dataIndex: "status", key: "status", render: (_, {status}) => {
            const color = status === "new" ? "cyan-inverse" : "blue-inverse";
            return <Tag color={color}>{status.toUpperCase()}</Tag>;
          },
        }, {
          title: "Amal", dataIndex: "operation", key: "operation", render: (_, {item_id}) => {
            return (<Button
              type="primary"
              danger
              size="small"
              shape="round"
              icon={<CloseOutlined/>}
              onClick={() => handleOpenCloseOrderItemModal({
                salesOrderId: id, salesOrderItemId: item_id,
              })}
            />);
          },
        },]}
    ...
    <MainCol>
      ...
      >
        {isLoading ? (<div className={styles.loadingCard}>
